...
func main() {
	...
	if err != nil {
		log.Printf("warn: failed to connect to architect DB: %v", err)
	} else {
	...
	app := server.New(server.Options{AllowedOrigins: cfg.AllowedOrigins, DB: conn, Env: cfg.Env, CoreAPIBase: cfg.CoreAPIBase})
	addr := ":" + cfg.Port
